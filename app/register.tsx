...
const ILLNESS_OPTIONS = ['치매', '천식', '당뇨', '심장질환', '뇌질환'];
...
export default function RegisterScreen() {
  ...
  const {
    ...
    defaultValues: {
      ...
      age: '', height: '', weight: '', illnesses: [], medications: '',
    },
  });
  ...
  const handleToggleIllness = (illness: string) => {
    const next = selectedIllnesses.includes(illness)
      ? selectedIllnesses.filter(i => i !== illness)
      : [...selectedIllnesses, illness];
    setValue('illnesses', next);
  };
  ...
  return (
    ...
    >
      ...
      >
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              <View style={styles.passwordContainer}>
                ...
                <TouchableOpacity onPress={() => setShowPasswordConfirm(!showPasswordConfirm)}>
                  <Ionicons name={showPasswordConfirm ? 'eye-off' : 'eye'} size={22} color="#666" />
                </TouchableOpacity>
              </View>
            )}
          />
        ...
        {/* ── 기저질환 선택 ── */}
        <View style={{ marginBottom: 16 }}>
          <Text style={styles.label}>보유 기저질환</Text>
          <View style={styles.chipGrid}>
            {ILLNESS_OPTIONS.map((ill) => {
              const selected = selectedIllnesses.includes(ill);
              return (
                <TouchableOpacity
                  key={ill}
        ...
        <Field
          name="medications"
          label="복약 중인 약 종류"
          placeholder="예) 혈압약, 당뇨약, 종합 비타민 (쉼표 구분)"
          control={control}
          errors={errors}
        />
